...
class TransformerEncoderRPR(Module):
    ...
    def forward(self, src, mask=None, img_encoding=None, src_key_padding_mask=None):
...
class TransformerEncoderLayerRPR(Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, use_img_encoding=True, er_len=None):
        ...
        self.use_img_encoding = use_img_encoding
        
        if use_img_encoding:
            self.linear_img = Linear(d_model, d_model)
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None, img_encoding=None):
        #img_encoding = torch.zeros(src.shape).cuda() + img_encoding
        #if not self.use_img_encoding or (self.use_img_encoding and img_encoding is None):
        #    img_residue = 0.
        #else:
        #    img_residue = self.linear_img(img_encoding)
        
        src2 = self.self_attn(src, src, src, attn_mask=src_mask,
                              key_padding_mask=src_key_padding_mask)[0]
        
        src = src + self.dropout1(src2) #+ img_encoding
        src = self.norm1(src)
